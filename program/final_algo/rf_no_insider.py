...
PRICES_AND_SENTIMENT = "full_merged_data.csv"
...
semifull_df["ticker"] = semifull_df["ticker"].astype(str)
semifull_df["date"] = pd.to_datetime(semifull_df["date"], format="mixed")
...
start_index = 0 #data.index[data["date"] >= pd.Timestamp("2020-03-01")][0]
end_index = len(X)#data.index[data["date"] < pd.Timestamp("2021-01-01")][-1]
