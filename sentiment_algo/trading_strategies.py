...
def run_svm_strategy(test_data):
    """
...
def run_buy_and_hold_strategy(test_data):
    """
...
def run_raw_svc_strategy(test_data, svc_buy_threshold=0.5, svc_sell_threshold=-0.5):
    """
    ...
    for ticker in test_data['ticker'].unique():
        ...
        for index, row in ticker_df.iterrows():
            ...
            if svc > svc_buy_threshold: # Buy
                trade_amount = cash * 0.10
                cash -= trade_amount
                stock_value += trade_amount
            elif svc < svc_sell_threshold: # Sell
                trade_amount = stock_value * 0.10
                stock_value -= trade_amount
                cash += trade_amount
